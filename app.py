...
def init_session():
    ...
    # Initialize step tracking for hints and attempts
    session['step_status'] = {
        'matrix': {'attempts': 0, 'hint_used': False},
        'rank': {'attempts': 0, 'hint_used': False},
        'indep': {'attempts': 0, 'hint_used': False},
        'relation': {'attempts': 0, 'hint_used': False}
    }
...
def check_matrix():
    ...
    status = session.get('step_status', {})['matrix']
    ...
    if np.allclose(user_matrix, true_matrix, atol=1e-4):
        if 'matrix' not in session.get('completed_steps', []):
            ...
            if status['hint_used']:
                mark = 0.0
            elif status['attempts'] > 0:
                mark = 0.5
            
            session['score'] = session.get('score', 0) + mark
            steps = session.get('completed_steps', [])
            steps.append('matrix')
        ...
        if status['attempts'] >= 2:
            ...
            return jsonify({
                "correct": False, 
                "show_answer": True,
                "expected": true_matrix.tolist(),
                "message": "Incorrect after 2 attempts. Showing correct matrix."
            })
        else:
            return jsonify({
                "correct": False, 
...
def check_rank():
    ...
    status = session.get('step_status', {})['rank']

    if user_rank == true_rank:
        if 'rank' not in session.get('completed_steps', []):
            mark = 1.0
            if status['hint_used']: mark = 0.0
            elif status['attempts'] > 0: mark = 0.5
            ...
            return jsonify({"correct": True, "score": session['score'], "rank": true_rank})
        else:
            return jsonify({"correct": True, "score": session['score'], "rank": true_rank})
    else:
        status['attempts'] += 1
        ...
        if status['attempts'] >= 2:
            ...
            return jsonify({
                ...
                "show_answer": True,
                "expected": true_rank,
                "message": f"Incorrect after 2 attempts. Rank is {true_rank}.",
                "steps": steps,
            ...
            return jsonify({
                "correct": False,
                "show_hint_button": True,
                "message": "Incorrect calculation. Try again or get a hint!"
            })
...
def check_independence():
    ...
    is_indep = session.get('is_independent')
    status = session.get('step_status', {})['indep']
    ...
    if is_correct:
        ...
        if status['attempts'] >= 2:
            ...
            return jsonify({
                "correct": False,
                "show_answer": True,
                "message": f"Incorrect. After 2 attempts: Reason is {reason}",
                "is_independent": is_indep
            })
        else:
            return jsonify({
                ...
                "show_hint_button": True,
                "is_independent": is_indep,
                "message": "Wrong conclusion. Re-check the rank condition!"
            })
...
def check_relation():
    ...
    A = np.array(session.get('matrix_a'))
    status = session.get('step_status', {})['relation']
    ...
    if is_zero_vector and is_nontrivial:
        ...
        if status['attempts'] >= 2:
            ...
            return jsonify({
                "correct": False,
                "show_answer": True,
                "message": "Incorrect attempt limit reached. Showing solution.",
            ...
            return jsonify({
                "correct": False,
                "show_hint_button": True,
                "message": "Relation is incorrect. Try again or get a hint!"
            })

@app.route('/api/get_hint', methods=['POST'])
def get_hint():
    step = request.json.get('step')
    status = session.get('step_status', {})
    
    if step in status:
        status[step]['hint_used'] = True
        session['step_status'] = status
        
    hints = {
        'matrix': "Matrix A is constructed by placing your vectors as columns. The first vector becomes the first column, second becomes the second, and so on.",
        'rank': "Reduced Row Echelon Form (RREF) reveals the rank. The rank is simply the total number of non-zero rows (rows with at least one 1) in that final matrix.",
        'indep': "Compare your Rank result with the number of vectors (k). If Rank = k, they are Independent. If Rank < k, they are Dependent.",
        'relation': "To find the relation, treat the vectors as a system of equations (Ax = 0) and solve for the variables c1, c2... using the row-reduced matrix."
    }
    
    return jsonify({"hint": hints.get(step, "No hint available for this step.")})
